...
import type { AnalystEmpty } from '../types'

export function AnalystEmptyState({ data }: { data: AnalystEmpty }) {
  ...
  const tone = data.reason === 'no_meta' ? 'glass-info' : 'glass-warning'
  const iconColor = data.reason === 'no_meta' ? 'text-blue-300' : 'text-amber-300'
  const Icon = data.reason === 'no_meta' ? Plug : RefreshCw
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-2xl p-8 text-center', tone)}>
      ...
      <div className="relative">
        ...
        <h2 className="mt-4 text-xl font-bold tracking-tight text-white">El Analyst necesita datos</h2>
        <p className="mx-auto mt-2 max-w-md text-sm text-white/60">{data.message}</p>

        {data.cta.action === 'connect_meta' ? (
